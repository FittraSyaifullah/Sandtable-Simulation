import { AgentTurn, DATASET_VERSION, ENSEMBLE_SAMPLES, MODEL_VERSION, Nation, ScenarioConfig, SimulationEvent, SimulationResult, WeeklyFrame } from "./sandtable";
...
function resolve(config: ScenarioConfig, nationA: Nation, nationB: Nation, sampleSeed: string, includeDetail: boolean, agentTurns: AgentTurn[]) {
  ...
  for (let week = 0; week <= config.duration; week += 1) {
    ...
    if (week > 0) {
      ...
      aSupply = Math.max(0, aSupply - (1.7 + tempoFactor * 1.9) * supplyAAgent + config.sideA.support / 90 + config.sideA.reinforcements / 110);
      bSupply = Math.max(0, bSupply - (1.7 + tempoFactor * 1.9) * supplyBAgent + config.sideB.support / 90 + config.sideB.reinforcements / 110);
      if (engaged) {
        ...
        aStrength = Math.max(2, aStrength - tempoFactor * readinessA * (2.2 + 7 * powerB / total));
        bStrength = Math.max(2, bStrength - tempoFactor * readinessB * (2.2 + 7 * powerA / total));
      } else {
...
export function runSimulation(config: ScenarioConfig, nations: Nation[], agentTurns: AgentTurn[] = []): SimulationResult {
  ...
  const canonical = resolve(config, nationA, nationB, config.seed, true, agentTurns);
  const samples = Array.from({ length: ENSEMBLE_SAMPLES }, (_, index) => resolve(config, nationA, nationB, `${config.seed}:${index}`, false, agentTurns));
  ...
  const configKey = JSON.stringify({ config, agentTurns });
  return {
    ...
    agentTurns,
    assumptions: ["National capability is represented through governed aggregate indicators.", "AI agents choose bounded strategic stance and resilience priorities; they do not generate operational plans.", "Formations move toward one abstract objective on weekly strategic ticks.", `${ENSEMBLE_SAMPLES} deterministic sample seeds produce the displayed ranges.`],
    limitations: ["Illustrative model output—not a forecast, intelligence assessment, or operational recommendation.", "Live intelligence, weapons employment, target selection, attack routes, and real unit locations are excluded.", "Aggregate inputs and model-generated agent choices cannot represent tactical context or establish predictive validity."],
  };
}
