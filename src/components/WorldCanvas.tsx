...
export function WorldCanvas({ config, nations, frame, week }: Props) {
  ...
  return (
    <div className="absolute inset-0 overflow-hidden bg-atlas">
      ...
      <div className="absolute left-1/2 top-[47%] h-[min(76vw,76vh)] w-[min(76vw,76vh)] -translate-x-1/2 -translate-y-1/2 rounded-full border border-white/10 bg-[#101a1d] shadow-[0_0_90px_rgba(249,115,22,.08)]">
        <svg className="h-full w-full" viewBox="0 0 100 100" role="img" aria-label="Abstract globe showing aggregate formations">
          ...
          <circle cx="50" cy="50" r="49" fill="#0d1618" stroke="#3c4748" strokeWidth=".45" />
          ...
          <path d={`M${aX} 55 Q50 38 ${bX} 55`} fill="none" stroke="#f97316" strokeWidth=".55" strokeDasharray="1.8 1.8" opacity=".85" />
          <circle cx="50" cy="46" r="3.4" fill="#f97316" opacity=".12" />
          <circle cx="50" cy="46" r="1.15" fill="#f97316" />
          <g transform={`translate(${aX} 55)`}><circle r="3.8" fill="#162426" stroke="#f97316" strokeWidth=".7" /><text y="1.2" textAnchor="middle" fill="#fff7ed" fontSize="3.2" fontWeight="700">A</text></g>
          <g transform={`translate(${bX} 55)`}><circle r="3.8" fill="#162426" stroke="#75a7aa" strokeWidth=".7" /><text y="1.2" textAnchor="middle" fill="#e7f3f3" fontSize="3.2" fontWeight="700">B</text></g>
        </svg>
      </div>
      <div className="absolute left-4 top-24 hidden rounded-2xl border border-white/10 bg-[#11191b]/90 p-2 shadow-2xl backdrop-blur md:flex md:flex-col md:gap-1">
      ...
      <div className="absolute bottom-28 left-4 hidden max-w-[250px] rounded-2xl border border-white/10 bg-[#11191b]/88 p-4 backdrop-blur md:block">
      ...
      <div className="absolute right-4 top-24 hidden w-52 space-y-2 lg:block">
        {[{ label: "SIDE A", nation: a, color: "#f97316" }, { label: "SIDE B", nation: b, color: "#75a7aa" }].map(side => <div key={side.label} className="rounded-2xl border border-white/10 bg-[#11191b]/88 p-3 backdrop-blur"><div className="flex items-center gap-2 text-[10px] tracking-[.18em] text-stone-500"><span className="h-2 w-2 rounded-full" style={{ background: side.color }} />{side.label}</div><p className="mt-2 truncate text-sm font-medium text-stone-100">{side.nation?.name}</p><p className="mt-1 text-[11px] text-stone-500">Aggregate formations</p></div>)}
      </div>
    </div>
  );
}
