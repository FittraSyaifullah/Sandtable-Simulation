...
export function WorldCanvas({ config, nations, frame, events = [], week }: Props) {
  ...
  return (
    <div className="absolute inset-0 overflow-hidden bg-atlas md:left-[480px] lg:left-[520px]">
      ...
      <div className="absolute left-4 top-24 hidden rounded-2xl border border-white/10 bg-[#0b0b0b]/90 p-2 shadow-2xl backdrop-blur md:flex md:flex-col md:gap-1"><Button onClick={()=>setZoom(value=>Math.min(1.3,+(value+.1).toFixed(1)))} disabled={zoom>=1.3} variant="ghost" size="icon" className="h-9 w-9 rounded-xl text-stone-400 hover:bg-white/5 hover:text-white" aria-label="Zoom in"><Plus className="h-4 w-4"/></Button><Button onClick={()=>setZoom(value=>Math.max(.8,+(value-.1).toFixed(1)))} disabled={zoom<=.8} variant="ghost" size="icon" className="h-9 w-9 rounded-xl text-stone-400 hover:bg-white/5 hover:text-white" aria-label="Zoom out"><Minus className="h-4 w-4"/></Button><Button onClick={()=>setZoom(1)} variant="ghost" size="icon" className="h-9 w-9 rounded-xl text-stone-400 hover:bg-white/5 hover:text-white" aria-label="Center globe"><LocateFixed className="h-4 w-4"/></Button><Button onClick={()=>setShowGrid(value=>!value)} aria-pressed={showGrid} variant="ghost" size="icon" className={`h-9 w-9 rounded-xl hover:bg-white/5 hover:text-white ${showGrid?"bg-yellow-400/10 text-yellow-300":"text-stone-400"}`} aria-label="Toggle map grid"><Layers3 className="h-4 w-4"/></Button></div>
