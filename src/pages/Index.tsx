...
export default function Index() {
  ...
  return <main className="relative h-[100dvh] min-h-[620px] overflow-hidden bg-[#0b1113] text-stone-100">
    <WorldCanvas config={config} nations={nations} frame={activeFrame} week={week}/>
    <header className="absolute inset-x-0 top-0 z-30 flex h-18 items-center gap-3 border-b border-white/8 bg-[#0b1113]/72 px-3 backdrop-blur-xl md:px-5">
      <button onClick={() => setSetupOpen(true)} className="flex items-center gap-3 rounded-xl focus:outline-none focus:ring-2 focus:ring-orange-500"><span className="grid h-10 w-10 place-items-center rounded-2xl border border-orange-500/30 bg-orange-500/10"><Globe2 className="h-5 w-5 text-orange-400"/></span><span className="text-left"><span className="block text-[15px] font-semibold tracking-tight">Sandtable</span><span className="hidden text-[9px] uppercase tracking-[.2em] text-stone-500 sm:block">Scenario laboratory</span></span></button>
    ...
    {!result && !setupOpen && <div className="absolute bottom-8 left-1/2 z-20 -translate-x-1/2"><Button onClick={()=>setSetupOpen(true)} className="h-12 rounded-2xl bg-orange-500 px-5 text-[#261307] hover:bg-orange-400"><Play className="mr-2 h-4 w-4"/>Configure study</Button></div>}
    ...
    <Dialog open={introOpen} onOpenChange={setIntroOpen}><DialogContent className="max-w-xl overflow-hidden rounded-[28px] border-white/10 bg-[#121a1c] p-0 text-stone-100"><div className="relative h-48 overflow-hidden border-b border-white/8 bg-[#0c1416]"><div className="atlas-grid absolute inset-0 opacity-50"/><div className="absolute left-1/2 top-1/2 grid h-28 w-28 -translate-x-1/2 -translate-y-1/2 place-items-center rounded-full border border-orange-500/30 bg-orange-500/[.06]"><Globe2 className="h-10 w-10 text-orange-400"/></div><div className="absolute bottom-4 left-5 rounded-full border border-white/10 bg-[#11191b] px-3 py-1 text-[9px] uppercase tracking-[.18em] text-stone-400">Inputs → seeded frames → ranges</div></div><div className="p-6"><DialogHeader><DialogTitle className="text-2xl tracking-tight">Explore assumptions, not predictions.</DialogTitle><DialogDescription className="mt-2 leading-relaxed text-stone-400">Sandtable is an educational laboratory for inspecting how aggregate capabilities, terrain, tempo, supply, and uncertainty interact in a simplified deterministic model.</DialogDescription></DialogHeader><div className="mt-5 grid gap-2 sm:grid-cols-3">{[[ShieldCheck,"Non-operational"],[Save,"Reproducible"],[Sparkles,"Reviewable AI"]].map(([Icon,label])=><div key={label as string} className="rounded-2xl border border-white/8 bg-white/[.025] p-3"><Icon className="h-4 w-4 text-orange-400"/><p className="mt-2 text-xs text-stone-300">{label as string}</p></div>)}</div><Button onClick={()=>{sessionStorage.setItem("sandtable-intro","dismissed");setIntroOpen(false)}} className="mt-5 h-11 w-full rounded-2xl bg-orange-500 text-[#251306] hover:bg-orange-400">Enter the laboratory<ChevronRight className="ml-2 h-4 w-4"/></Button></div></DialogContent></Dialog>

    <Dialog open={!!proposal || proposalLoading} onOpenChange={open=>!open&&setProposal(undefined)}><DialogContent className="max-w-2xl rounded-[28px] border-white/10 bg-[#121a1c] text-stone-100"><DialogHeader><DialogTitle>{proposalLoading?"Drafting a constrained proposal…":proposal?.title}</DialogTitle><DialogDescription className="text-stone-400">AI output never runs automatically. Review and explicitly apply it.</DialogDescription></DialogHeader>{proposalLoading?<div className="h-44 animate-pulse rounded-2xl bg-white/[.04]"/>:proposal&&<div><p className="rounded-2xl border border-white/8 bg-white/[.025] p-4 text-sm leading-relaxed text-stone-300">{proposal.summary}</p><div className="mt-4 grid gap-3 sm:grid-cols-2"><ProposalCell label="Objective" value={proposal.objective}/><ProposalCell label="Configuration" value={`${proposal.terrain} terrain · ${proposal.tempo} tempo · ${proposal.duration} weeks`}/></div><div className="mt-4"><p className="text-[10px] uppercase tracking-[.18em] text-stone-500">Sensitivity alternatives</p><ul className="mt-2 space-y-1 text-xs text-stone-400">{proposal.alternatives.map(item=><li key={item}>• {item}</li>)}</ul></div><p className="mt-4 rounded-2xl border border-amber-500/20 bg-amber-500/[.06] p-3 text-xs text-amber-200">{proposal.safetyNotice}</p><div className="mt-5 flex justify-end gap-2"><Button variant="ghost" onClick={()=>setProposal(undefined)} className="rounded-xl">Discard</Button><Button onClick={applyProposal} className="rounded-xl bg-orange-500 text-[#251306] hover:bg-orange-400">Apply for review</Button></div></div>}</DialogContent></Dialog>

    <Dialog open={libraryOpen} onOpenChange={setLibraryOpen}><DialogContent className="max-w-2xl rounded-[28px] border-white/10 bg-[#121a1c] text-stone-100"><DialogHeader><DialogTitle>Shared study library</DialogTitle><DialogDescription className="text-stone-500">Controlled demonstration workspace · all visitors can modify saved studies.</DialogDescription></DialogHeader><div className="max-h-[55vh] space-y-2 overflow-y-auto">{saved.length===0?<div className="rounded-2xl border border-dashed border-white/10 p-10 text-center text-sm text-stone-500">No studies saved yet.</div>:saved.map(item=><div key={item.id} className="flex items-center gap-3 rounded-2xl border border-white/8 bg-white/[.025] p-4"><button onClick={()=>openSaved(item)} className="min-w-0 flex-1 text-left"><p className="truncate text-sm font-medium text-stone-200">{item.name}</p><p className="mt-1 text-[11px] text-stone-500">{item.region_label} · {new Date(item.updated_at).toLocaleDateString()}</p></button><Button onClick={()=>navigate(`/share/${item.share_token}`)} variant="ghost" size="icon" className="rounded-xl text-stone-500" aria-label="Open shared report"><Share2 className="h-4 w-4"/></Button><Button onClick={()=>removeSaved(item)} variant="ghost" size="icon" className="rounded-xl text-stone-600 hover:text-red-300" aria-label="Delete study"><Trash2 className="h-4 w-4"/></Button></div>)}</div></DialogContent></Dialog>

    <Dialog open={methodOpen} onOpenChange={setMethodOpen}><DialogContent className="max-w-3xl rounded-[28px] border-white/10 bg-[#121a1c] text-stone-100"><DialogHeader><DialogTitle>Method & evidence registry</DialogTitle><DialogDescription className="text-stone-500">Versioned provenance for an inspectable educational model.</DialogDescription></DialogHeader><div className="grid gap-3 md:grid-cols-3"><MethodCell title="Weekly strategic ticks" text="One canonical replay resolves movement, adjacency, supply, strength, and objective state."/><MethodCell title="240 seeded samples" text="The same force layer reruns with deterministic sample seeds to produce central ranges."/><MethodCell title="Versioned provenance" text={`Model unified-force-0.3 · dataset ${activeDataset} · as of 2025-01-15.`}/></div><div className="rounded-2xl border border-white/8 bg-white/[.025] p-4"><div className="flex items-center justify-between"><p className="text-sm font-medium">Active evidence record</p><span className="rounded-full bg-emerald-500/10 px-2 py-1 text-[9px] uppercase tracking-wider text-emerald-300">Active</span></div><p className="mt-2 text-xs leading-relaxed text-stone-500">Aggregate Capability Reference 2025.1 · 8 governed nation records · macro indicators and normalized capability inputs. Governed local records remain available when reference queries fail.</p></div><div className="rounded-2xl border border-amber-500/20 bg-amber-500/[.05] p-4 text-xs leading-relaxed text-stone-400"><strong className="text-amber-200">Responsible-use boundary.</strong> Sandtable excludes real unit locations, targeting, strike planning, weapons optimization, attack routes, detection evasion, and operational recommendations. Calibration references are explanatory benchmarks, not claims of predictive validity.</div></DialogContent></Dialog>
  </main>;
}
